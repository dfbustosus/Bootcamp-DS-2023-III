...
assistant = client.beta.assistants.retrieve("asst_UoTWfetExbajw40Yp6CVoFsZ");
###################################################
# 5. Conversacion
stop= True;
## 5.1 Crear un hilo (thread)
    ...
    if run.status == "requires_action":
        ...
        for each_tool in tools_to_calls:
            ...
            if function_name =="gather_user_data":
                ...
                output= "Ahora pregunta sobre los detalles de la reserva"
            elif function_name == "gather_reservation_data":
                output = "Informale al usuario que la reserva esta agendada";
            elif function_name== "not_talk":
                if json.loads(function_arg)['fin'] == True: stop=False;
                output= "Genial que tengas un gran dia fue un gusto ayudarte hoy"
